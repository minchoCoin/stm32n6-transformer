...
class MultiHeadSelfAttentionDense(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads):
        ...
        self.q_proj = FullyConnected(d_model)
        self.k_proj = FullyConnected(d_model)
        self.v_proj = FullyConnected(d_model)
        self.out_proj = FullyConnected(d_model)
...
class FeedForwardSimple(tf.keras.layers.Layer):
    def __init__(self, d_model, d_ff):
        super().__init__()
        self.fc1 = FullyConnected(d_ff, activation='relu')
        self.fc2 = FullyConnected(d_model, activation=None)
...
def main():
    ...
    if args.use_layernorm:
        filename=f"v2_custom_vit_ln_im{args.img_size}_attdim{args.d_model}_depth{args.num_blocks}_head{args.num_heads}_ff{args.d_ff}.tflite"
    else:
        filename=f"v2_custom_vit_im{args.img_size}_attdim{args.d_model}_depth{args.num_blocks}_head{args.num_heads}_ff{args.d_ff}.tflite"

...
//__attribute__ ((section (".psram_bss")))
//__attribute__ ((aligned (32)))
//uint8_t crop_buffer[NN_WIDTH * NN_HEIGHT * NN_BPP];
...
static void PatchEmbedding(uint8_t* src, uint8_t* dst);
...
int main(void)
{
  ...
  while (1)
  {
     ...
     */
    //img_crop(dcmipp_out_nn, crop_buffer, pitch_nn, NN_WIDTH, NN_HEIGHT, NN_BPP);
    PatchEmbedding(dcmipp_out_nn,nn_in);
...
static void PatchEmbedding(uint8_t* src, uint8_t* dst)
{
    const int img_w = NN_WIDTH;
    const int img_h = NN_HEIGHT;
    const int channels = NN_BPP;

    const int num_patches_x = img_w / PATCH_SIZE;
    const int num_patches_y = img_h / PATCH_SIZE;
    ...
    for (int py = 0; py < num_patches_y; py++)
    {
        for (int px = 0; px < num_patches_x; px++)
        {
            ...
            int patch_y0 = py * PATCH_SIZE;
            int patch_x0 = px * PATCH_SIZE;

            // 패치 내부 16x16 픽셀 순회 (TF extract_patches와 동일한 순서: ky -> kx -> c)
            for (int ky = 0; ky < PATCH_SIZE; ky++)
            {
                int in_y = patch_y0 + ky;

                for (int kx = 0; kx < PATCH_SIZE; kx++)
                {
                    int in_x = patch_x0 + kx;


                    int in_offset = (in_y * img_w + in_x) * channels;
                    const uint8_t *pix = &src[in_offset];
